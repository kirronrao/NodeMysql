...
exports.getNodes =(req,res)=>{
  console.log('Nodes loaded')
    ...
    connection.query('SELECT * FROM nodes ', function (error, results, fields) {
      ...
      res.render('pages/nodes',{nodes:results});
      }
    })
 
}
exports.viewNode=(req,res)=>{

    var id = req.params.id;
   connection.query('SELECT * FROM nodes where id = ? ',[id], function (error, results, fields) {
      if (error) 
      {
        console.log('Error',error);
      }
      ...
      {
      console.log('results',results);
      res.render('pages/viewnode',{node:results[0]});
      }
    })
